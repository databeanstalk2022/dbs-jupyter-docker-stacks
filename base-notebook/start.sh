...
# the dbstalk user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the dbstalk user to the desired the user (NB_USER)
    if id dbstalk &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" dbstalk 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the dbstalk user:"
            _log "- username: dbstalk       -> ${NB_USER}"
            _log "- home dir: /home/dbstalk -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the dbstalk user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the dbstalk home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "dbstalk" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/dbstalk to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/dbstalk/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/dbstalk to /home/${NB_USER}!"
                _log "Attempting to symlink /home/dbstalk to /home/${NB_USER}..."
                if ln -s /home/dbstalk "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/dbstalk to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/dbstalk/"* ]]; then
...
# user we started as.
    ...
    dbstalk_UID="$(id -u dbstalk 2>/dev/null)"  # The default UID for the dbstalk user
    dbstalk_GID="$(id -g dbstalk 2>/dev/null)"  # The default GID for the dbstalk user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old dbstalk user to nayvoj ($(id -u dbstalk):$(id -g dbstalk))"
            ...
            sed --expression="s/^dbstalk:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/dbstalk:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "dbstalk" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/dbstalk. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/dbstalk ]]; then
        _log "WARNING: no write access to /home/dbstalk. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
